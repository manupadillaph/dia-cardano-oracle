...
import { makePaymentHookValidator } from "../core/contracts.js";
import {
  makeConfiguredLucid,
...
export async function paymentHookWithdraw(args: {
  ...
  reportProgress(`Loading payment-hook withdraw input from ${path.resolve(args.inputPath)}`);
  const input = await readPaymentHookWithdrawInput(path.resolve(args.inputPath));
...
async function readPaymentHookWithdrawInput(inputPath: string): Promise<PaymentHookWithdrawInput> {
  const raw = await readFile(inputPath, "utf8");
  return JSON.parse(raw) as PaymentHookWithdrawInput;
}
